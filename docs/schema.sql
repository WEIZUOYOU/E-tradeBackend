...
CREATE TABLE `review` (
    `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
    `trade_id` BIGINT NOT NULL COMMENT '交易 ID',
    `reviewer_id` BIGINT NOT NULL COMMENT '评价者 ID',
    `reviewee_id` BIGINT NOT NULL COMMENT '被评价者 ID',
    ...
    `product_image` VARCHAR(500) COMMENT '商品图片路径',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
